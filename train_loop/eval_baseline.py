#!/usr/bin/env python3
"""
Loads val.jsonl (or train_loop/data/val.jsonl), sends each example to OpenRouter
...
Usage:
  python train_loop/eval_openrouter_baseline.py
  python train_loop/eval_openrouter_baseline.py --max_samples 200 --model anthropic/claude-3-sonnet
  python train_loop/eval_openrouter_baseline.py --data_path train_loop/data/val.jsonl --output results.json
"""
...
from train_loop.config import VAL_JSONL
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate OpenRouter model on harm classification val set")
    parser.add_argument(
        "--data_path",
        type=Path,
        default=VAL_JSONL,
        help="Path to val.jsonl (default: train_loop/data/val.jsonl)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default=os.environ.get("OPENROUTER_MODEL_EVAL"),
        help="OpenRouter model ID (default: OPENROUTER_MODEL or anthropic/claude-3-haiku)",
    )
    parser.add_argument(
        "--max_samples",
        type=int,
